...
for i = 1:length(t)
    ...
    Rt = norm([yt(i)-bsy,xt(i)-bsx]);
    Rmdot = Vp0*cos(alphap(i)-thetam);
    Rtdot = Vt0*cos(alphat(i)-thetat);
    % thetamdot = vp0*sin(alphap(i)-thetam)/Rm;
    thetatdot = Vt0*sin(alphat(i)-thetat)/Rt;
    thetatdotdot = Vt0*(cos(alphat(i)-thetat)*(A_target/Vt0 - thetatdot)/Rt - sin(alphat(i)-thetat)*Rtdot/Rt^2);

    A_cmd(i) = 1*Rm*(thetat-thetam) + Rm*(thetatdotdot) + 2*Rmdot*thetatdot;
end
...
function dX = system(t,X,bs,A_target)
    ...
    Rt = norm([X(8)-bs(2),X(7)-bs(1)]);
    Rmdot = X(9)*cos(X(3)-thetam);
    Rtdot = X(10)*cos(X(6)-thetat);
    % thetamdot = X(9)*sin(X(3)-thetam)/Rm;
    thetatdot = X(10)*sin(X(6)-thetat)/Rt;
    thetatdotdot = X(10)*(cos(X(6)-thetat)*(A_target/X(10) - thetatdot)/Rt - sin(X(6)-thetat)*Rtdot/Rt^2);

    
    dX = zeros(length(X),1);
    dX(1) = X(10)*cos(X(6)-X(2)) - X(9)*cos(X(3)-X(2));
    dX(2) = (1/X(1))*(X(10)*sin(X(6)-X(2)) - X(9)*sin(X(3)-X(2)));
    A_cmd = 1*Rm*(thetat-thetam) + Rm*(thetatdotdot) + 2*Rmdot*thetatdot;
    dX(3) = A_cmd/X(9);
    dX(4) = X(9)*cos(X(3));
    dX(5) = X(9)*sin(X(3));
    dX(6) = A_target/X(10); 
