...
[t, y] = ode45(@(t, y)los_CLOS(t, y, VT, VP, 0, K), tspan, y20, options);
...
dthetaT = VT*sin(alphaT - thetaT)./Rt;

ddthetaT = (VT*cos(alphaT-thetaT)*0 - 2*dthetaT.*VT.*cos(alphaT-thetaT))./Rt;

aP = K*VP*Rp.*(thetaT - thetaP) + Rp.*ddthetaT + 2*dthetaT.*VP.*cos(alphaP-thetaP);
...
function dydt = los_CLOS(t, y, VT, VP, cT, K)
    ...
    dthetaT = VT*sin(y(4) - thetaT)/Rt;
    ddthetaT = (VT*cos(y(4)-thetaT)*dydt(4) - 2*dthetaT*VT*cos(y(4)-thetaT))/Rt;

    dydt(3) = K*Rp*(thetaT - thetaP) + Rp*ddthetaT/VP + 2*dthetaT*VP*cos(y(3)-thetaP)/VP;
